...
function pvtDisplayDataInDiv() {
    ...
    if (LDivRadioBtn.checked) {
        ...
        pvtGenerateHeaders('div', 'div min-w-full table');
    } else if (LTableRadioBtn.checked) {
        // pvtGenerateTable();
        pvtGenerateHeaders('table', 'min-w-full border-collapse');
    }
}
...
function pvtGenerateHeaders(p_strType, p_strClassName) {
    const LDataHeadElement = pvtCreateCustomTagElement(p_strType, p_strClassName);
    let LTableChildContainer;
    if (p_strType === 'div') {
        pvtCreateDivHeaderIfNeeded();
        LDataHeadElement.appendChild(GListHeadElement)
        LTableChildContainer = document.createElement('div');
    } else {
        pvtCreateTableHeaderIfNeeded();
        LDataHeadElement.appendChild(GDataHeadElement);
...
function pvtCreateTableHeaderIfNeeded() {
    if (!GDataHeadElement) {
        ...
        LTableHeadingName.forEach(heading => {
            let LTableHeadCell = pvtCreateTableHeadCell(LTableHeadClass, null, heading);
...
function pvtCreateCustomTagElement(p_StrTagName, p_strClassName) {
